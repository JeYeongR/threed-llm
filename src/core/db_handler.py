...
def delete_posts_by_company(company_name: str) -> int:
    """
    지정된 회사의 모든 포스트를 데이터베이스에서 효율적으로 삭제합니다.
    company_name은 Company Enum의 멤버 이름 문자열 (예: "KAKAO")입니다.
    """
    try:
        Company[company_name.upper()]
    except KeyError:
        logger.error(f"'{company_name}'은(는) 유효한 Company 이름이 아닙니다.")
        return 0

    deleted_count = 0
    try:
        with db_session_manager() as db:
            logger.info(f"{company_name} 회사의 포스트 삭제를 시작합니다...")

            posts_to_delete_query = db.query(DBCompanyPost).filter(
                DBCompanyPost.company == company_name.upper()
            )

            deleted_count = posts_to_delete_query.delete(synchronize_session=False)
        ...
        logger.error(
            f"{company_name} 회사 포스트 삭제 중 오류 발생: {e}", exc_info=True
        )
        return 0
